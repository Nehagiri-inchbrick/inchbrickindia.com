...
// The raw CSS was injected right before </head>
// We need to wrap it in <style> tags.
// Let's find the start of the injected CSS.
const cssStartMarker = '/* Contact Page Styles */';
if (contactHtml.includes(cssStartMarker) && !contactHtml.includes('<style>\n  /* Contact Page Styles */')) {
  // It was inserted without <style>
  contactHtml = contactHtml.replace(cssStartMarker, '<style>\n  ' + cssStartMarker);
  // It was inserted right before </head>, so the end of the CSS is right before </head>
  // But wait, when I did replace('</head>', contactCSS), I didn't put </head> back!
  // Wait: contactHtml = contactHtml.replace('</head>', contactCSS); 
  // Oh no, I completely replaced </head> with the CSS! This means the head was never closed, and the CSS text just spilled into the body.
  
  // Let's fix this properly.
  // First, let's see if </head> is missing.
  if (!contactHtml.includes('</head>')) {
     // The CSS is at the very end of the head section, right before <body> or <main>
     // Let's just find the end of the CSS string and add </style></head>
     // Actually, it's easier to just read the file, extract the CSS (everything from /* Contact Page Styles */ to <body... or <main...)
     // Or I can just re-generate it correctly from index.html! That is much safer.
  }
}

// Safer approach: Re-generate contact.html correctly from index.html
...
let newContactHtml = headAndHeader + contactMain + footerAndScripts;

// Update the contact nav link
newContactHtml = newContactHtml.replace(/<a href="#">Contact<\/a>/g, '<a href="contact.html">Contact</a>');

// Inject the CSS properly WITH <style> tags and keeping </head>
const contactCSS = `
<style>
  /* Contact Page Styles */
  .contact-main {
...
console.log('Fixed contact.html CSS injection.');
